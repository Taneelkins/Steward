 ...
 */
async function postRobloxFixMessage(db, guild, record, errorMsg) {
    const config = db.getGuildConfig(guild.id);
...
function buildFixDiscordComponents(caseId) {
...
function buildFixRobloxComponents(caseId) {
    return new ActionRowBuilder().addComponents(new ButtonBuilder()
    ...
    if (record.actionName === "appeal" && record.appealResult === "accepted") {
        ...
        try {
            if (appealTypeLower.includes("ban")) {
                ...
                if (jailedRow) {
                    const savedRoleIds = JSON.parse(jailedRow.saved_role_ids_json);
 ...
 */
export async function handleFixPunishmentButton(db, interaction) {
    const { customId } = interaction;
    if (!customId.startsWith("fix_discord_id:") &&
 ...
 */
export async function handleFixPunishmentModal(db, interaction) {
    const { customId } = interaction;
    if (!customId.startsWith("update_discord_id:") && !customId.startsWith("update_roblox_id:"))
        return false;
    if (!interaction.guild)
        return false;
    await interaction.deferReply({ ephemeral: true });
    ...
    if (action === "update_discord_id") {
        ...
        await interaction.message?.edit({ components: [] }).catch(() => null);
        await interaction.editReply(`✅ Case #${caseId} Discord ID updated to <@${newDiscordId}> (\`${newDiscordId}\`).\nClick **⚡ Execute Punishment** on the log to retry.`);
        return true;
    }
