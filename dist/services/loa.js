...
import { postToConfiguredChannel, safeDm } from "../utils/discord.js";
...
// ── Embed builders ────────────────────────────────────────────────────────────
export function buildLoaRequestEmbed(request) {
    return new EmbedBuilder()
        .setTitle("📋 LOA Request")
        .setColor(colors.voidPurple)
        .addFields({ name: "Staff Member", value: `<@${request.userId}> (${request.username})`, inline: true }, { name: "Duration", value: request.durationText, inline: true }, {
        name: "Expires",
...
export function buildLoaLogEmbed(request) {
    return new EmbedBuilder()
        ...
        value: request.expiresAt ? discordTimestamp(request.expiresAt, "D") : "No end date",
        inline: true
    }, { name: "Approved By", value: request.approvedBy ? `<@${request.approvedBy}>` : "Unknown", inline: true }, { name: "Reason", value: request.reason })
        .setFooter({ text: `LOA #${request.id}` })
        .setTimestamp();
}
export function buildLoaApprovalButtons(loaId) {
...
// ── Button handler ────────────────────────────────────────────────────────────
export async function handleLoaButton(db, interaction) {
    ...
    if (isNaN(loaId) || (action !== "approve" && action !== "deny"))
        return false;
...
// ── Approve ───────────────────────────────────────────────────────────────────
async function approveLoa(db, guild, request, approverId) {
    ...
    db.run(`INSERT INTO quota_exemptions (guild_id, user_id, reason, expires_at, created_by, created_at)
     ...
     ON CONFLICT(guild_id, user_id) DO UPDATE SET
       ...
       created_at = excluded.created_at`, guild.id, request.userId, `LOA #${request.id}: ${request.reason}`, request.expiresAt, approverId, nowIso());
    // Post to LOA log channel
    const config = db.getGuildConfig(guild.id);
    const updatedRequest = { ...request, approvedBy: approverId, status: "approved" };
            ...
            await safeDm(discordUser, {
                embeds: [
                    new EmbedBuilder()
                        .setTitle("LOA Approved")
                        .setColor(0x2ecc71)
                        .setDescription(`Your Leave of Absence request in **${guild.name}** has been approved.\n\n` +
                        `**Duration:** ${request.durationText}\n` +
                        expiresLine + "\n\n" +
...
// ── Deny ──────────────────────────────────────────────────────────────────────
async function denyLoa(db, guild, request, denierId) {
            ...
            await safeDm(discordUser, {
                embeds: [
                    new EmbedBuilder()
                        .setTitle("LOA Denied")
                        .setColor(0xe74c3c)
                        .setDescription(`Your Leave of Absence request in **${guild.name}** was not approved.\n\n` +
