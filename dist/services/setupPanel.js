...
function buildPanelEmbed(db, guildId) {
    ...
    const ch = (id, label, opt = false) => id ? `✅ **${label}:** <#${id}>` : opt ? `⬜ **${label}:** Not set` : `❌ **${label}:** Not set`;
    const rl = (id, label, opt = false) => id ? `✅ **${label}:** <@&${id}>` : opt ? `⬜ **${label}:** Not set` : `❌ **${label}:** Not set`;
    const tog = (val, label) => `${val ? "✅" : "❌"} **${label}:** ${val ? "Enabled" : "Disabled"}`;
    const tierLabels = [
...
// ── Panel payload ─────────────────────────────────────────────────────────────
export function buildSetupPanel(db, guildId, userId) {
    const config = db.getGuildConfig(guildId);
    const togBtn = (key, label, on) => new ButtonBuilder()
...
function buildRolesModal(db, guildId) {
    const staffRoles = db.listStaffRoles(guildId);
    const get = (key) => staffRoles.find((r) => r.key === key)?.roleId ?? null;
    return new ModalBuilder()
        .setCustomId("cfg_panel:modal:roles")
        .setTitle("Edit Staff Roles")
        .addComponents(ri("staff", "Staff Role", get("staff")), ri("juniorMod", "Junior Mod Role", get("juniorMod")), ri("mod", "Normal Mod Role", get("mod")), ri("headMod", "Head Mod Role", get("headMod")), ri("communityManager", "Community Manager Role", get("communityManager")));
}
function buildLogChannelsModal(db, guildId) {
    const g = (name) => db.getActionLogChannelId(guildId, name);
    return new ModalBuilder()
        .setCustomId("cfg_panel:modal:log_ch")
        .setTitle("Edit Log Channels")
        .addComponents(ci("ban", "Ingame Ban Log", g("ban")), ci("strike", "Strike Log", g("strike")), ci("restore", "Restore Log", g("restore")), ci("discord", "Discord Log", g("discord")), ci("ticket", "Ticket Log", g("ticket")));
}
function buildCoreChannelsModal(db, guildId) {
    const c = db.getGuildConfig(guildId);
    return new ModalBuilder()
        .setCustomId("cfg_panel:modal:core_ch")
        .setTitle("Edit Core Channels")
        .addComponents(ci("alerts", "Alerts", c.alertChannelId), ci("audit", "Audit Log", c.auditChannelId), ci("approval", "CM Approval", c.approvalChannelId), ci("junior_help", "Junior Help", c.juniorHelpChannelId), ci("quota", "Quota Board", c.quotaChannelId));
}
function buildExtraChannelsModal(db, guildId) {
    const c = db.getGuildConfig(guildId);
    return new ModalBuilder()
...
export async function handleSetupPanelButton(db, interaction) {
    ...
    if (section === "toggle") {
        const config = db.getGuildConfig(guildId);
        if (key === "interactive_log")
            db.updateGuildConfig(guildId, { interactive_log_enabled: config.interactiveLogEnabled ? 0 : 1 });
        else if (key === "cm_approval")
            db.updateGuildConfig(guildId, { approval_enabled: config.approvalEnabled ? 0 : 1 });
        else if (key === "points")
            db.updateGuildConfig(guildId, { points_enabled: config.pointsEnabled ? 0 : 1 });
        else if (key === "quota")
            db.updateGuildConfig(guildId, { quota_enabled: config.quotaEnabled ? 0 : 1 });
    ...
    if (section === "modal") {
        let modal;
        if (key === "roles")
            modal = buildRolesModal(db, guildId);
        else if (key === "log_ch")
            modal = buildLogChannelsModal(db, guildId);
        else if (key === "core_ch")
...
export async function handleSetupPanelModal(db, interaction) {
    if (!interaction.customId.startsWith("cfg_panel:modal:"))
        return false;
    if (!interaction.guild)
        return false;
    const member = interaction.member;
    ...
    if (section === "roles") {
        const keys = ["staff", "juniorMod", "mod", "headMod", "communityManager"];
    ...
    if (section === "log_ch") {
        for (const name of ["ban", "strike", "restore", "discord", "ticket"]) {
         ...
         VALUES (?, ?, ?, ?, ?)
         ON CONFLICT(guild_id, action_name) DO UPDATE SET channel_id = excluded.channel_id, updated_at = excluded.updated_at`, guildId, name, id, nowIso(), nowIso());
        }
