...
export function buildPendingTicketEmbed(pending) {
    return new EmbedBuilder()
        ...
        .setColor(0x3498db)
        .addFields({ name: "Ticket", value: pending.ticketId ?? `Pending #${pending.id}`, inline: true }, { name: "Type", value: pending.ticketType, inline: true }, { name: "Due", value: `${discordTimestamp(pending.dueAt, "F")}\n${discordTimestamp(pending.dueAt, "R")}`, inline: true }, { name: "Opener", value: pending.openerUserId ? `<@${pending.openerUserId}>` : "Unknown", inline: true }, { name: "Transcript", value: transcriptFieldValue(pending.transcriptUrl), inline: false })
...
export async function handleTicketButton(db, interaction) {
    if (!interaction.guild || !interaction.member || !interaction.customId.startsWith("ticketlog:"))
        return false;
    ...
    if (action === "dismiss") {
        ...
        db.run("UPDATE pending_ticket_logs SET status = 'dismissed', admin_notes = ? WHERE guild_id = ? AND id = ?", `Dismissed by ${interaction.user.id}`, interaction.guild.id, pending.id);
    ...
    if (action === "noaction") {
        ...
        db.run("UPDATE pending_ticket_logs SET status = 'logged', logged_case_id = ? WHERE guild_id = ? AND id = ?", record.id, interaction.guild.id, pending.id);
...
export function resolveTicketAction(db, guildId, ticketType, noAction = false) {
