...
export function buildCommands(options = {}) {
    ...
    const commands = [
        ...
        new SlashCommandBuilder()
            ...
            .addUserOption((option) => option.setName("member").setDescription("The staff member to promote.").setRequired(true))
            .setDefaultMemberPermissions(headDefault),
        new SlashCommandBuilder()
            .setName("demote")
            .setDescription("Demote a staff member down one tier (CM → Head → Senior → Mod → Junior) in all linked servers.")
            .addUserOption((option) => option.setName("member").setDescription("The staff member to demote.").setRequired(true))
            .addStringOption((option) => option.setName("reason").setDescription("Reason for the demotion (sent to the member via DM)."))
            .setDefaultMemberPermissions(headDefault),
        new SlashCommandBuilder()
            .setName("fire")
            .setDescription("Remove all staff roles from a member in both linked servers.")
            .addUserOption((option) => option.setName("member").setDescription("The staff member to fire.").setRequired(true))
            .addStringOption((option) => option.setName("reason").setDescription("Reason for termination (optional)."))
            .setDefaultMemberPermissions(headDefault),
        new SlashCommandBuilder()
            ...
            .addUserOption((option) => option.setName("member").setDescription("The staff member.").setRequired(true))
            .addStringOption((option) => option.setName("roblox_username").setDescription("Their exact Roblox username.").setRequired(true))
            .setDefaultMemberPermissions(headDefault),
...
function editCommand() {
    return new SlashCommandBuilder()
        ...
        .addStringOption((option) => option.setName("roblox_user").setDescription("Roblox username of the player.").setRequired(true))
        .addStringOption((option) => option.setName("stat").setDescription("Stat path to edit, e.g. Stats.Elo or Player.Clan.").setRequired(true))
        .addStringOption((option) => option.setName("value").setDescription("New value — numbers, true/false, or text.").setRequired(true))
        .addStringOption((option) => option.setName("game").setDescription("Game name (only needed if multiple games are configured)."))
        .setDefaultMemberPermissions(communityDefault);
}
