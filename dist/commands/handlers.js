...
import { ActionRowBuilder, AttachmentBuilder, ButtonBuilder, ButtonStyle, ChannelType, EmbedBuilder, ModalBuilder, PermissionFlagsBits, TextInputBuilder, TextInputStyle } from "discord.js";
import { formatMultiplier, formatPoints, listOrNone, pointsToMilli, truncate } from "../utils/format.js";
import { canUseAccess, caseLinkComponents, commandDeniedMessage, configSummaryEmbed, getTextChannel, hasCanRegisterRole, isAdminMember, postToConfiguredChannel, requireAdmin, requireMod } from "../utils/discord.js";
...
async function handleRegister(interaction, { db }, member) {
    ...
    await postToConfiguredChannel(guild, config.staffRegistrationChannelId, {
        embeds: [
            new EmbedBuilder()
                ...
                .setColor(0x2ecc71)
                .addFields({ name: "Staff Member", value: `<@${member.id}>`, inline: true }, { name: "Staff Role", value: `<@&${provisioned.roles.staff.id}>`, inline: true })
                .setTimestamp()
        ],
...
async function handleLog(interaction, { db }, member) {
...
async function handleLogEdit(interaction, { db }, member) {
    const caseId = interaction.options.getInteger("case_id", true);
    await startEditLog(interaction, db, member, caseId);
}
async function handleQuickLog(interaction, { db }, member, actionName, actionDisplayName) {
...
async function submitTypedLog(interaction, db, member, actionName, actionDisplayName) {
    const guild = interaction.guild;
...
async function handlePointAlias(interaction, { db }, member, sign) {
...
async function handleCheckpoints(interaction, { db }, member) {
    assertPointsEnabled(db, interaction.guild.id);
    ...
    const embed = new EmbedBuilder()
        ...
        .setColor(0x5865f2)
        .addFields({ name: "Your Points", value: formatPoints(total), inline: true }, { name: "Active Registered Staff", value: truncate(listOrNone(lines), 1000), inline: false })
        .setTimestamp();
    await interaction.reply({ embeds: [embed], ephemeral: true });
}
async function handleMultiplierView(interaction, { db }, member) {
    assertPointsEnabled(db, interaction.guild.id);
    await requireMod(db, member);
...
async function handleConfig(interaction, { db }, member) {
    await requireAdmin(db, member);
    const guild = interaction.guild;
    ...
    if (subcommand === "behavior") {
        ...
        const promoteDemoteRole = interaction.options.getRole("promote_demote_role");
        let promoteDemoteRoleIdsJson;
...
async function handleAction(interaction, { db }, member) {
    await requireAdmin(db, member);
    const guild = interaction.guild;
    const subcommand = interaction.options.getSubcommand();
    ...
    if (subcommand === "upsert") {
        ...
        if (pointsEnabled && rawPoints === null && !existing)
            throw new Error("Set points when creating a new action preset.");
        ...
        db.run(`INSERT INTO action_presets (
      ...
      ON CONFLICT(guild_id, name) DO UPDATE SET
        ...
        updated_at = excluded.updated_at`, guild.id, name, rawName, points, noActionPoints === null ? existing?.noActionPointsMilli ?? 0 : pointsToMilli(noActionPoints), strikes, evidenceRequired ? 1 : 0, enabled ? 1 : 0, existing?.createdAt ?? timestamp, timestamp);
    ...
    if (subcommand === "points") {
        ...
        if (!action)
            throw new Error(`Action preset "${name}" does not exist.`);
       ...
       WHERE guild_id = ? AND name = ?`, points, noActionPoints === null ? null : pointsToMilli(noActionPoints), endsAt, reason, interaction.user.id, timestamp, guild.id, name);
    ...
    if (subcommand === "clear-points") {
       ...
       WHERE guild_id = ? AND name = ?`, timestamp, guild.id, name);
...
async function handleCase(interaction, { db }, member) {
    const guild = interaction.guild;
    ...
    if (group === "history") {
        ...
        const rows = db.all(`SELECT id, action_name, reason, created_at, status FROM moderation_cases WHERE guild_id = ? AND ${column} = ? ORDER BY id DESC LIMIT 10`, guild.id, target.id);
...
async function handlePoints(interaction, { db }, member) {
    const guild = interaction.guild;
    assertPointsEnabled(db, guild.id);
    const subcommand = interaction.options.getSubcommand();
    ...
    const rows = db.all("SELECT moderator_user_id, COALESCE(SUM(amount_milli), 0) AS points FROM point_ledger WHERE guild_id = ? GROUP BY moderator_user_id ORDER BY points DESC LIMIT 20", guild.id);
    const lines = rows.map((row, index) => `${index + 1}. <@${row.moderator_user_id}> - ${formatPoints(row.points)}`);
    await interaction.reply({ content: truncate(listOrNone(lines), 1900), ephemeral: true });
}
async function handleStrikes(interaction, { db }, member) {
    const target = interaction.options.getUser("target", true);
    const total = getStrikeTotal(db, interaction.guild.id, target.id);
    const rows = db.all("SELECT case_id, amount, created_at FROM strikes WHERE guild_id = ? AND target_user_id = ? AND active = 1 ORDER BY id DESC LIMIT 10", interaction.guild.id, target.id);
...
async function handleWarnings(interaction, { db }, member) {
    await requireMod(db, member);
    const guild = interaction.guild;
    const target = interaction.options.getUser("target", true);
    ...
    const rows = db.all("SELECT id, case_id, reason, moderator_user_id, created_at FROM warnings WHERE guild_id = ? AND discord_target_id = ? ORDER BY id DESC LIMIT 10", guild.id, target.id);
...
function resolveRobloxGame(db, guildId, nameOption) {
    ...
    if (nameOption) {
        ...
        if (!game)
            throw new Error(`No Roblox game found named "${nameOption}". Use \`/roblox list\` to see configured games.`);
        return game;
    }
    if (games.length === 1)
        return games[0];
...
async function handleIngameBan(interaction, { db }, member) {
    await requireMod(db, member);
    const guild = interaction.guild;
    ...
    await interaction.editReply(`✅ **${robloxUser.name}** (ID: ${robloxUser.id}) banned from **${game.name}**.\n` +
        `**Duration:** ${durationLabel}\n**Reason:** ${reason}${caseNote}`);
}
async function handleIngameUnban(interaction, { db }, member) {
    if (!canUseAccess(db, member, "head"))
        throw new Error(commandDeniedMessage("head"));
    const guild = interaction.guild;
    const robloxUsername = interaction.options.getString("roblox_user", true).trim();
...
async function handleEdit(interaction, { db }, member) {
    if (!canUseAccess(db, member, "community"))
        throw new Error(commandDeniedMessage("community"));
    await interaction.deferReply({ ephemeral: true });
    const guild = interaction.guild;
    ...
    if (!readResult.success) {
        if (readResult.notFound) {
            ...
            await interaction.editReply(`⚠️ **${robloxUser.name}** has no saved data yet (never joined). Sent a live broadcast instead — ` +
    ...
    if (activeSession) {
        ...
        await interaction.editReply(`✅ **${robloxUser.name}** is currently online — edit applied live.\n` +
    ...
    if (!ok) {
        await interaction.editReply(`❌ Stat path \`${statPath}\` is invalid — an intermediate key doesn't exist.\n` +
    ...
    if (!writeResult.success) {
        await interaction.editReply(`❌ Edit failed when saving: ${writeResult.error}\n` +
    ...
    await interaction.editReply(`✅ **${robloxUser.name}**'s data saved directly to DataStore.\n` +
        `\`${statPath}\` → \`${String(value)}\`\n` +
...
function buildAutoPunishPanel(db, guildId) {
...
async function handleAutoPunish(interaction, { db }, member) {
    if (!canUseAccess(db, member, "head"))
        throw new Error(commandDeniedMessage("head"));
    await interaction.reply({ ...buildAutoPunishPanel(db, interaction.guild.id), ephemeral: true });
}
export async function handleAutoPunishButton(db, interaction) {
    if (!interaction.customId.startsWith("autopunish:"))
        return false;
    if (!interaction.guild)
        return false;
    const member = interaction.member;
    ...
    if (parts[1] !== "toggle")
        return false;
    const key = parts[2];
    if (!PUNISH_TYPES.some((t) => t.key === key))
        return false;
    const config = db.getGuildConfig(interaction.guild.id);
    const disabled = [...config.autoPunishDisabled];
...
async function handleMultiplier(interaction, { db }, member) {
    ...
    if (endsAtText && !endsAt)
        throw new Error("Could not parse ends_at. Use an ISO date/time.");
    const reason = interaction.options.getString("reason", true);
...
async function handleQuota(interaction, { db }, member) {
    const guild = interaction.guild;
    const subcommand = interaction.options.getSubcommand();
    const group = interaction.options.getSubcommandGroup(false);
    if (group === "exempt") {
    ...
    if (subcommand === "role-set") {
        ...
        const required = interaction.options.getInteger("required_logs", true);
        const grace = interaction.options.getInteger("grace_logs") ?? 0;
        db.run(`INSERT INTO role_quotas (guild_id, role_id, required_logs, grace_logs, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(guild_id, role_id) DO UPDATE SET required_logs = excluded.required_logs, grace_logs = excluded.grace_logs, updated_at = excluded.updated_at`, guild.id, role.id, required, grace, nowIso(), nowIso());
        const config = db.getGuildConfig(guild.id);
        if (config.quotaPeriodStart && config.quotaPeriodEnd)
            await snapshotRoster(db, guild, config.quotaPeriodStart, config.quotaPeriodEnd);
    ...
    if (subcommand === "history") {
        ...
        const lines = rows.map((row) => {
            const statuses = JSON.parse(row.status_json);
...
async function handleQuotaExempt(interaction, db, member) {
    await requireAdmin(db, member);
    const guild = interaction.guild;
    const subcommand = interaction.options.getSubcommand();
    if (subcommand === "add") {
        ...
        if (expiresText && !expires)
            throw new Error("Could not parse expires_at. Use an ISO date/time.");
        db.run(`INSERT INTO quota_exemptions (guild_id, user_id, reason, expires_at, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT(guild_id, user_id) DO UPDATE SET reason = excluded.reason, expires_at = excluded.expires_at, created_by = excluded.created_by, created_at = excluded.created_at`, guild.id, moderator.id, reason, expires?.toISOString() ?? null, interaction.user.id, nowIso());
    ...
    const rows = db.all("SELECT user_id, reason, expires_at FROM quota_exemptions WHERE guild_id = ?", guild.id);
...
async function handleStaff(interaction, { db }, member) {
    ...
    const cases = db.get("SELECT COUNT(*) AS count FROM moderation_cases WHERE guild_id = ? AND moderator_user_id = ? AND status = 'active'", guild.id, moderator.id)?.count ?? 0;
    const flags = db.get("SELECT COUNT(*) AS count FROM moderation_cases WHERE guild_id = ? AND moderator_user_id = ? AND flags <> '' AND status = 'active'", guild.id, moderator.id)?.count ?? 0;
    const fields = [
...
async function handleAudit(interaction, { db }, member) {
    ...
    const rows = db.all("SELECT * FROM audit_events WHERE guild_id = ? ORDER BY id DESC LIMIT ?", interaction.guild.id, limit);
...
async function handleBackup(interaction, { db, env }, member) {
    await requireAdmin(db, member);
    await interaction.deferReply({ ephemeral: true });
...
async function handleExport(interaction, { db, env }, member) {
    await requireAdmin(db, member);
    await interaction.deferReply({ ephemeral: true });
...
function readCaseTarget(interaction) {
...
function assertPointsEnabled(db, guildId) {
...
function caseReplyText(prefix, caseId, awardedPointsMilli, pointsEnabled) {
    return pointsEnabled
        ? `${prefix} case #${caseId} for ${formatPoints(awardedPointsMilli)} points.`
        : `${prefix} case #${caseId}.`;
}
function readConfigRoleUpdates(interaction) {
    ...
    const legacyMod = interaction.options.getRole("mod_role");
    const legacyAdmin = interaction.options.getRole("admin_role");
    ...
    return {
        staffRoles,
        canRegisterRole: interaction.options.getRole("can_register_role"),
        legacyModRole: (legacyMod ?? interaction.options.getRole("normal_mod_role") ?? interaction.options.getRole("junior_mod_role")),
        legacyAdminRole: (legacyAdmin ?? interaction.options.getRole("head_mod_role") ?? interaction.options.getRole("community_manager_role"))
    };
}
function upsertStaffRole(db, guildId, key, role) {
    ...
    db.run(`INSERT INTO staff_roles (guild_id, role_id, role_key, name, level, is_admin, created_at, updated_at)
     ...
     ON CONFLICT(guild_id, role_id) DO UPDATE SET
       ...
       updated_at = excluded.updated_at`, guildId, role.id, key, role.name, spec.level, spec.isAdmin ? 1 : 0, timestamp, timestamp);
}
function readConfigActionLogChannelUpdates(interaction) {
    ...
    return mappings
        ...
        .filter((update) => Boolean(update.channel));
}
function upsertActionLogChannel(db, guildId, actionName, channelId) {
    const timestamp = nowIso();
    db.run(`INSERT INTO action_log_channels (guild_id, action_name, channel_id, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(guild_id, action_name) DO UPDATE SET channel_id = excluded.channel_id, updated_at = excluded.updated_at`, guildId, actionName.toLowerCase(), channelId, timestamp, timestamp);
}
function readSetupRoleOverrides(interaction) {
    return {
        staff: interaction.options.getRole("staff_role"),
        communityManager: interaction.options.getRole("community_manager_role"),
        headMod: interaction.options.getRole("head_mod_role"),
        seniorMod: interaction.options.getRole("senior_mod_role"),
        mod: interaction.options.getRole("normal_mod_role"),
        juniorMod: interaction.options.getRole("junior_mod_role")
    };
}
function readCanRegisterRoleOverride(interaction) {
    return interaction.options.getRole("can_register_role");
}
function readSetupChannelOverrides(interaction) {
    return {
...
async function ensureProvisionedForStaffRole(db, guild) {
    const config = db.getGuildConfig(guild.id);
...
function saveProvisionedConfig(db, guildId, provisioned, ownerUserId) {
...
function savedChannelIdsFromConfig(db, guildId) {
    const config = db.getGuildConfig(guildId);
    return {
...
function normalizeStoredRoleKey(key, name) {
    if (isStaffRoleKey(key))
        return key;
    const normalized = name.toLowerCase().replace(/\s+/g, "-");
    if (normalized === "staff")
        return "staff";
    if (normalized === "junior-mod")
        return "juniorMod";
    if (normalized === "mod" || normalized === "normal-mod")
        return "mod";
    if (normalized === "senior-mod")
        return "seniorMod";
    if (normalized === "head-mod")
        return "headMod";
...
async function handleConfigCheck(interaction, db, guildId) {
    ...
    const ch = (id, label, optional = false) => id ? `✅ ${label}: <#${id}>` : optional ? `⬜ ${label}: Not set (optional)` : `❌ ${label}: Not set`;
    const role = (id, label, optional = false) => id ? `✅ ${label}: <@&${id}>` : optional ? `⬜ ${label}: Not set (optional)` : `❌ ${label}: Not set`;
    const user = (id, label) => id ? `✅ ${label}: <@${id}>` : `❌ ${label}: Not set`;
    const bool = (value, label) => value ? `✅ ${label}` : `❌ ${label}: Not configured`;
    const channelLines = [
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: "Channels", value: channelLines.join("\n"), inline: false }, { name: "Roles", value: roleLines.join("\n"), inline: false }, { name: "Other", value: otherLines.join("\n"), inline: false })
 ...
 */
function findTierConfig(key, staffRoles) {
 ...
 */
function getMemberTierKey(member, staffRoles) {
 ...
 */
async function applyTierChange(guild, userId, fromKey, toKey, staffRoles, reason) {
...
async function handlePromoteOrDemote(interaction, context, actor, direction) {
    const { db } = context;
    const guild = interaction.guild;
...
async function stripAllStaffRoles(guild, userId, staffRoles, reason) {
    const member = await guild.members.fetch(userId).catch(() => null);
    if (!member)
        return { removed: [], notInServer: true };
    const removed = [];
...
async function handleFire(interaction, context, actor) {
    ...
    const { removed: mainRemoved, notInServer: mainMissing } = await stripAllStaffRoles(guild, targetUser.id, staffRoles, fireReason);
    ...
    if (config.linkedGuildId) {
        ...
        if (linkedGuild) {
            const linkedStaffRoles = db.listStaffRoles(linkedGuild.id);
            const { removed: linkedRemoved, notInServer: linkedMissing } = await stripAllStaffRoles(linkedGuild, targetUser.id, linkedStaffRoles, fireReason);
...
async function handleSetupSecondary(interaction, context, _member) {
    ...
    if (subcommand === "roles") {
        ...
        for (const { key, option } of tierOptions) {
            const explicit = interaction.options.getRole(option);
            if (explicit) {
            ...
            const byName = guild.roles.cache.find((r) => !r.managed && aliases.some((alias) => r.name.toLowerCase() === alias)) ?? null;
            if (byName) {
        ...
        if (results.length === 0) {
            await interaction.editReply("❌ No roles found. Either pass roles explicitly (e.g. `junior_mod:@Junior Mod`) or make sure the server has roles whose names match the tier names (e.g. \"Junior Mod\", \"Mod\", \"Senior Mod\", \"Head Mod\", \"Community Manager\").");
            return;
        }
    ...
    let jailCategory = guild.channels.cache.find((c) => c.type === ChannelType.GuildCategory && c.name === "Jailed");
    ...
    let jailChat = jailCategory.children.cache.find((c) => c.name === "jailed-chat");
    if (!jailChat) {
    ...
    let jailAnnouncements = jailCategory.children.cache.find((c) => c.name === "jail-announcements");
    ...
    const otherCategories = guild.channels.cache.filter((c) => c.type === ChannelType.GuildCategory && c.id !== jailCategory.id);
    for (const category of otherCategories.values()) {
        await category.permissionOverwrites.create(jailedRole, { ViewChannel: false }, { reason: "Jail setup: deny jailed role view" }).catch(() => null);
    }
    // 6. Add deny-ViewChannel overwrite for Jailed role on top-level channels (no parent)
    const topLevelChannels = guild.channels.cache.filter((c) => !c.parentId && c.type !== ChannelType.GuildCategory);
    for (const channel of topLevelChannels.values()) {
        if ("permissionOverwrites" in channel) {
            await channel.permissionOverwrites.create(jailedRole, { ViewChannel: false }, { reason: "Jail setup: deny jailed role view" }).catch(() => null);
        }
    }
...
async function replyError(interaction, error) {
    ...
    const options = { content: `Error: ${message}`, ephemeral: true };
    if (interaction.deferred || interaction.replied) {
        await interaction.editReply({ content: options.content }).catch(() => null);
    }
    else {
