...
import type { GuildConfig, QuotaMemberStatus, QuotaReport } from "../types.js";
...
export function buildQuotaStatusEmbed(config: GuildConfig) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      { name: "Required Logs", value: String(config.quotaRequiredLogs), inline: true },
      { name: "Grace", value: `${config.quotaGraceLogs} logs`, inline: true },
      { name: "Status", value: config.quotaEnabled ? "Enabled" : "Disabled", inline: true },
      { name: "Schedule", value: `${dayName(config.quotaCheckDay)} at ${String(config.quotaCheckHour).padStart(2, "0")}:${String(config.quotaCheckMinute).padStart(2, "0")} ${config.timezone}` },
      { name: "Ends", value: `${discordTimestamp(end, "F")}\n${discordTimestamp(end, "R")}` }
    )
    .setFooter({ text: "This message is edited when quota settings or periods change." })
    .setTimestamp();
}

export function buildQuotaReportEmbed(report: QuotaReport, title = "Quota Period Ended") {
  ...
  const missedLines = missed.map(
    (status) => `<@${status.userId}> - ${status.loggedActions}/${status.requiredLogs} logs, missing ${status.missing}`
  );
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "Missed", value: truncate(listOrNone(missedLines), 1000), inline: false },
      { name: "Close", value: truncate(listOrNone(closeLines), 1000), inline: false },
      { name: "Exempt", value: truncate(listOrNone(exemptLines), 1000), inline: false },
      { name: "Summary", value: `${report.statuses.length} staff checked. ${missed.length} missed, ${close.length} close.` }
    )
    .setTimestamp(new Date(report.createdAt));
}

export async function ensureQuotaPeriod(db: AppDatabase, guild: Guild) {
...
export async function snapshotRoster(db: AppDatabase, guild: Guild, periodStart: string, periodEnd: string) {
  const config = db.getGuildConfig(guild.id);
  const staffRoles = db.listStaffRoles(guild.id);
  const staffRoleIds = new Set(staffRoles.map((role) => role.roleId));
  if (config.modRoleId) staffRoleIds.add(config.modRoleId);
  if (staffRoleIds.size === 0) return;
  const roleQuotas = db.all<RoleQuotaRow>("SELECT * FROM role_quotas WHERE guild_id = ?", guild.id);
  ...
  db.run(
    "DELETE FROM quota_roster_snapshots WHERE guild_id = ? AND period_start = ? AND period_end = ?",
    guild.id,
    periodStart,
    periodEnd
  );
  ...
  for (const member of members.values()) {
    if (member.user.bot || !member.roles.cache.some((role) => staffRoleIds.has(role.id))) continue;
    const quota = pickQuotaForMember(member, roleQuotas, config, staffRoles);
    db.run(
      `INSERT INTO quota_roster_snapshots (
...
export function buildQuotaReport(db: AppDatabase, guildId: string, periodStart: string, periodEnd: string): QuotaReport {
  const snapshots = db.all<SnapshotRow>(
    "SELECT user_id, role_id, required_logs, grace_logs FROM quota_roster_snapshots WHERE guild_id = ? AND period_start = ? AND period_end = ?",
    guildId,
    periodStart,
    periodEnd
  );
  const exemptions = db.all<ExemptionRow>(
    "SELECT user_id, reason, expires_at FROM quota_exemptions WHERE guild_id = ?",
    guildId
  );
  ...
  const statuses: QuotaMemberStatus[] = snapshots.map((snapshot) => {
    const loggedActions =
      db.get<{ count: number }>(
...
export async function closeQuotaPeriod(db: AppDatabase, guild: Guild, actorUserId = "system", reason = "Scheduled quota close") {
  await ensureQuotaPeriod(db, guild);
  const config = db.getGuildConfig(guild.id);
  if (!config.quotaPeriodStart || !config.quotaPeriodEnd) return null;
  const report = buildQuotaReport(db, guild.id, config.quotaPeriodStart, config.quotaPeriodEnd);

  db.run(
    "INSERT INTO quota_reports (guild_id, period_start, period_end, status_json, created_at) VALUES (?, ?, ?, ?, ?)",
    guild.id,
    report.periodStart,
    report.periodEnd,
...
export async function maybeSendQuotaWarning(db: AppDatabase, guild: Guild) {
  const config = db.getGuildConfig(guild.id);
  if (!config.quotaEnabled || !config.quotaPeriodStart || !config.quotaPeriodEnd || config.quotaWarningSentAt) return;
  const warningAt = new Date(new Date(config.quotaPeriodEnd).getTime() - config.quotaWarningHours * 60 * 60 * 1000);
  if (Date.now() < warningAt.getTime()) return;
  const report = buildQuotaReport(db, guild.id, config.quotaPeriodStart, config.quotaPeriodEnd);
...
export async function processQuotaTimers(db: AppDatabase, guild: Guild) {
  await ensureQuotaPeriod(db, guild);
  const config = db.getGuildConfig(guild.id);
  if (!config.quotaEnabled || !config.quotaPeriodEnd) return;
...
export async function upsertQuotaStatusMessage(db: AppDatabase, guild: Guild) {
  const config = db.getGuildConfig(guild.id);
  if (!config.quotaChannelId) return;
...
export async function setQuotaSchedule(
  db: AppDatabase,
  guild: Guild,
  actorId: string,
  values: { checkDay: number; checkHour: number; checkMinute: number; timezone: string; frequencyDays: number }
) {
  const start = nowIso();
  const end = computeNextQuotaEnd({
...
function pickQuotaForMember(member: GuildMember, roleQuotas: RoleQuotaRow[], config: GuildConfig, staffRoles: Array<{ roleId: string; level: number }> = []) {
...
export function quotaHistory(db: AppDatabase, guildId: string, limit = 5) {
  return db.all<{ id: number; period_start: string; period_end: string; status_json: string; created_at: string }>(
    "SELECT * FROM quota_reports WHERE guild_id = ? ORDER BY id DESC LIMIT ?",
    guildId,
    limit
  );
}

export function quotaLeaderboard(db: AppDatabase, guildId: string, periodStart?: string | null, periodEnd?: string | null) {
  if (periodStart && periodEnd) {
    return db.all<{ moderator_user_id: string; logs: number; points: number }>(
...
export function buildLeaderboardEmbed(rows: { moderator_user_id: string; logs: number; points: number }[], title = "Quota Leaderboard", showPoints = true) {
