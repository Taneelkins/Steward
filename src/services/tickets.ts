...
export async function processOverdueTickets(db: AppDatabase, guild: Guild) {
  db.run(
    "UPDATE pending_ticket_logs SET status = 'overdue' WHERE guild_id = ? AND status IN ('pending', 'needs_review') AND due_at <= ?",
    guild.id,
    nowIso()
  );
}

export function resolveTicketAction(db: AppDatabase, guildId: string, ticketType: string) {
